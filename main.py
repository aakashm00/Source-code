...
def process_uploaded_image(filename):
    ...
    if file_extension in {'jpg', 'jpeg'}:
        process = subprocess.Popen(["python", "detect.py", '--source', os.path.join(app.config['UPLOAD_FOLDER'], filename), "--weights", "best.pt"], shell=True)
        process.wait()
...
def analysis1():
    file = os.path.join(img, 'Default.jpeg')
    return render_template('./analysis2.html', image=file)

@app.route('/analysis3')
def analysis2():
    file = os.path.join(img, 'Default.jpeg')
    return render_template('./analysis3.html', image=file)

@app.route('/analysis4')
def analysis3():
    file = os.path.join(img, 'Default.jpeg')
    return render_template('./analysis4.html', image=file)
